...
describe('Rate limiting (e2e)', () => {
  ...
  it('blocks brute-forcing the operator credentials on POST /admin/companies past the configured limit', async () => {
    // CompaniesController is throttled to 30 requests / 15min (see
    // companies.controller.ts). Fire past that with wrong credentials and
    ...
    for (let i = 0; i < 35; i++) {
      const res = await request(app.getHttpServer())
        ...
        .set('x-admin-username', 'definitely-wrong-username')
        .set('x-admin-password', 'definitely-wrong-password')
        .send({ name: `Brute Force Attempt ${i}` });
      results.push(res.status);
    }

    // First 30 should be rejected as unauthorized (wrong credentials, not yet
